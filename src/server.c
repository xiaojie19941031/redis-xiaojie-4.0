...
int serverCron(
    ...
    // 如果重写进程以及bgsave重写进程都不存在并且此时安排了重写进行重写
...
/**
 * 初始化一组文件描述符,以侦听指定的“端口”绑定Redis服务器配置中指定的地址
 *
 * 用于接收集群中其它节点的TCP连接
 *
 * @param port
...
void initServer(void) {
    ...
    // 当需要注意模块中被阻止的客户端时,为用于唤醒事件循环的管道注册一个可读事件
