 ...
 *   数据分布实现
 *
 *      使用虚拟槽[16384,0~16383],公式slot=CRC16（key）&16383[为什么使用16384个槽?]
 *
 *         节省redis集群中节点之间交换信息使用的带宽[防止心跳包过大浪费带宽]
 *
 *   限制
 *
 ...
 *
 *      key作为数据分区的最小粒度,因此不能将一个大的键值对象hash、list等映射到不同的节点
 *
 *      不支持多数据库空间,只是有db0
 *
 ...
 *   集群搭建以及原理
 *
 *      1、节点握手[Gossip协议](clusterCron中执行)
 *
 *
 *
 *
 ...
 * @param n
 */
int clusterCountNonFailingSlaves(
        clusterNode *n) {
 ...
 * @param cport
 */
int clusterHandshakeInProgress(
        char *ip,
        int port,
        int cport) {
    ...
    while ((de = dictNext(di)) != NULL) {
        ...
        if (!strcasecmp(node->ip, ip) && node->port == port && node->cport == cport) break;
    }
    dictReleaseIterator(di);
    return de != NULL;
}
...
void clusterWriteHandler(
    ...
    if (nwritten <= 0) {
        serverLog(LL_DEBUG, "I/O error writing to node link: %s", strerror(errno));
...
int clusterNodeIsInGossipSection(
        clusterMsg *hdr,
        int count,
        clusterNode *n) {
...
/**
 * 将PING或PONG数据包发送到指定的节点
 *
 * @param link
 * @param type
 */
void clusterSendPing(
        clusterLink *link,
        int type) {
...
void clusterCron(
    ...
    while ((de = dictNext(di)) != NULL) {
        ...
        if (node->link == NULL) {
            ...
            // 如果此时节点状态为CLUSTER_NODE_MEET就发送meet消息
...
void clusterCommand(
    ...
    if (!strcasecmp(c->argv[1]->ptr, "meet") && (c->argc == 4 || c->argc == 5)) {
        ...
        if (getLongLongFromObject(c->argv[3], &port) != C_OK) {
            addReplyErrorFormat(c, "Invalid TCP base port specified: %s", (char *) c->argv[3]->ptr);
            return;
        }
        if (c->argc == 5) {
            if (getLongLongFromObject(c->argv[4], &cport) != C_OK) {
                addReplyErrorFormat(c, "Invalid TCP bus port specified: %s", (char *) c->argv[4]->ptr);
                return;
            }
        } else {
            // 基础端口号上+10000
        ...
        clusterDoBeforeSleep(CLUSTER_TODO_UPDATE_STATE | CLUSTER_TODO_SAVE_CONFIG);
        addReply(c, shared.ok);
        // 槽迁移[cluster setslot{slot}importing{sourceNodeId}]
        ...
        // migrating[让源节点准备迁移数据],判断当前要迁移的槽是否为自己的
        if (!strcasecmp(c->argv[3]->ptr, "migrating") && c->argc == 5) {
            if (server.cluster->slots[slot] != myself) {
                addReplyErrorFormat(c, "I'm not the owner of hash slot %u", slot);
                return;
            }
            // 查询目标节点是否是自己已经建立连接的[是否为集群中的节点]
